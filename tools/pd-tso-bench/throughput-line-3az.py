...
if __name__ == "__main__":
    ...
    for i in range(len(axs)):
        ...
        if i == len(axs)-1:
            ...
            axs[i].set_xlabel("Time")
            axs[i].set_ylabel('Timestamp / s')
            axs[i].yaxis.set_label_coords(-0.09, 1.4)
        else:
            axs[i].set_xticks(time_ticks)
            axs[i].set_xticklabels("")
    axs[0].plot(taas_time, taas_rps, c='xkcd:violet', linewidth=2)
    axs[0].set_title('TaaS', loc='right')
    axs[1].plot(tso_time, tso_rps, c='xkcd:rust', label="throughput", linewidth=2)
    axs[1].set_title('TiDB-PD', loc='right')

    # fig.legend(ncol=4, loc='upper center')
    fig.subplots_adjust(top=.92)
    fig.savefig('throughput-line-3az.pdf',bbox_inches='tight', format='pdf')